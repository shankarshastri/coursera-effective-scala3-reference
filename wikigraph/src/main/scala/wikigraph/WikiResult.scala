...
case class WikiResult[A](val value: Future[Either[Seq[WikiError], A]]):
  ...
  def flatMap[B](f: A => WikiResult[B])(using ExecutionContext): WikiResult[B] = 
    WikiResult(value.flatMap(e => {
      val k1 = e match {
        case Left(e) => Future.successful[Either[Seq[WikiError], B]](Left(e))
        case Right(res) => f(res).value
      }
      k1
    }))
  ...
  def zip[B](that: WikiResult[B])(using ExecutionContext): WikiResult[(A, B)] =
    def zipEithersAcc(a: Either[Seq[WikiError], A], b: Either[Seq[WikiError], B]): Either[Seq[WikiError], (A, B)] =
      (a, b) match
        case (Right(a1), Right(b1)) => Right((a1, b1))
        case (Left(a1), Right(b1)) => Left(a1)
        case (Right(a1), Left(b1)) => Left(b1)
        case (Left(a1), Left(b1)) => Left((a1 ++ b1))
...
object WikiResult:
  ...
  def traverse[A, B](ls: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    ls.map(f).foldLeft(WikiResult.successful[Seq[B]](Seq.empty[B]))((a,b) => {
      a.zip(b.map(Seq(_))).map(e => e._1 ++ e._2)
    })
