...
trait EncoderInstances:
  ...
  given Encoder[Boolean] = Encoder.fromFunction(e => Json.Bool(e))
  /**
...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] = 
    Decoder.fromPartialFunction { case Json.Num(e) if e.isValidInt => e.toInt }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      e => {
        e match {
          case Json.Arr(e) => e.foldLeft(Option(List.empty[A]))((a,b) =>  decoder.decode(b).flatMap(b => a.map(e => b :: e))).map(e => e.reverse)
          case _ => None
        }
      }
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    ...
    .transform[Person](e => Person(e._1, e._2))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = ObjectEncoder.field[List[Person]]("people").transform(e => e.people)
  given Decoder[Contacts] = Decoder.field[List[Person]]("people").transform(e => Contacts(e))
