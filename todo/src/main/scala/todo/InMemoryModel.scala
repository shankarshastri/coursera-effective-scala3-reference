...
object InMemoryModel extends Model:
  ...
  def complete(id: Id): Option[Task] =
    val m = idStore.get(id)
    m match
      case Some(i) => 
        val updatedState = i.copy(state = State.completedNow)
        idStore.put(id, updatedState)
        Some(updatedState)
      case _ => None
  ...
  def tags: Tags =
    Tags(idStore.flatMap(e => e._2.tags.toSet).toSet.toList)

  def tasks(tag: Tag): Tasks =
    Tasks(idStore.filter(e => e._2.tags.contains(tag)))
